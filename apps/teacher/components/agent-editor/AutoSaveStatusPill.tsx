...
export function AutoSaveStatusPill({ status }: { status: AutoSaveStatus }) {
  ...
  if (status.kind === "saving") {
    return (
      <Pill cls="bg-white text-ink border-rule">
        <Spinner /> Saving…
      </Pill>
    );
  }
...
function Pill({
  ...
  return (
    <div
      role="status"
      aria-live="polite"
      className={`fixed bottom-4 right-4 z-50 border rounded-full px-3 py-1.5 text-xs shadow-sm flex items-center gap-2 ${cls}`}
    >
      {children}
    </div>
  );
