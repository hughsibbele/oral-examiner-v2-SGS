...
- **Soft-archive needs partial unique indices, not table-level uniques.** The initial schema put a global `unique (exam_template_id, student_id)` constraint on `exam_sessions`, which blocked M2b.5c.3's retake-with-grace flow: archiving a sub-1-min completion to `state='excluded'` left the row in place and re-using `(template, student)` violated the constraint. Fixed 2026-05-19 via migration `20260519130751` (replace with partial unique on non-excluded states) and later re-keyed by `20260519131123` to `(canvas_assignment_id, student_id)`. **Pattern to copy** for any new table where soft-archive + retake matters.

- **Student-flow file map (M2b.5c, 2026-05-19).** `lib/exam/resolve.ts` returns a discriminated union (`no_binding` / `no_agent` / `not_on_roster` / `ok`) — service-role throughout since post-recursion-fix RLS has no student-scoped read path on `exam_templates`. `lib/exam/session.ts` classifies prior sessions and soft-archives. `lib/exam/start-exam.ts` is the Start server action; `lib/exam/teacher-actions.ts` is `resetExamSession`. The `/exam/<aid>` page renders four screens (not-configured / not-on-roster / completion-blocked / ready); `/exam/<aid>/run` is a 5d-bound stub today. **All student-context DB reads must use the service-role admin client** — already documented on `lib/supabase/admin.ts`, repeated here for muscle memory.

- **`exam_sessions` can point at preset OR template directly.** The 20260519131123 migration mirrored the `exam_template_bindings` shape — CHECK constraint enforces exactly-one-of `(exam_template_id, personality_preset_id)`. Materializing a teacher-owned template at session-start time would have re-introduced the "your custom templates" hub pollution the 20260518022646 design pivot explicitly avoided. Future code that reads `exam_sessions` needs to dereference both columns.
